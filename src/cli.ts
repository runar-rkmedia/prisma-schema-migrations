...
const configPath = findUp.sync(['.prisma-schema-migrator', '.prisma-schema-migrator.json'])
...
config.migrationsDir = config['migrations-dir'] || path.join(config.schemaDir, 'migrations')
...
const init = async () => {
  ...
  const {
    _: [command],
    to, down, force, name: _name, latest
  } = argv as any
